...
program_version_link="https://raw.githubusercontent.com/darklost/auto_shell/master/frps/version.sh"
str_install_shell="https://raw.githubusercontent.com/darklost/auto_shell/master/frps/install-frps.sh"
shell_update(){
    fun_info "clear"
...
pre_install(){
    ...
    if [ -s ${str_program_dir}/${program_name} ] && [ -s ${program_init} ]; then
        ...
        defIP=$(curl -s https://api.ip.sb/ip)
...
uninstall)
    ...
    ;;
update)
    update_program_server 2>&1 | tee /root/${program_name}-update.log
    ;;
*)
    fun_info
